    ...
    <div class="py-12 bg-orange-50/20">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-2xl sm:rounded-3xl border border-orange-100 p-8">
                    ...
                    <table class="w-full text-left border-separate border-spacing-y-3">
                        ...
                        <tbody>
                            ...
                            <tr class="bg-white hover:bg-orange-50/50 transition-all shadow-sm group">
                                <td class="p-4 border-y border-l rounded-l-2xl text-center font-black text-gray-400 group-hover:text-orange-500">
                                    {{ $index + 1 }}
                                </td>
                                <td class="p-4 border-y">
                                    <div class="flex items-center gap-4">
                                        <div class="w-10 h-10 bg-orange-100 rounded-xl flex items-center justify-center text-orange-600 font-black shadow-inner">
                                            {{ substr($doctor->nama_dokter, 0, 1) }}
                                        </div>
                                        <div>
                                            <div class="font-black text-gray-800 uppercase leading-tight">{{ $doctor->nama_dokter }}</div>
                                            <div class="text-[9px] text-gray-400 font-bold mt-0.5 italic uppercase">ID Dokter: #D-00{{ $doctor->id }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="p-4 border-y">
                                    <span class="inline-block px-3 py-1 bg-orange-100 text-orange-700 rounded-full text-[10px] font-black uppercase tracking-tighter shadow-sm">
                                        {{ $doctor->spesialis }}
                                    </span>
                                </td>
                                <td class="p-4 border-y border-r rounded-r-2xl text-center">
                                    <div class="flex justify-center items-center gap-2">
                                        <a href="{{ route('doctors.edit', $doctor->id) }}" class="p-2 bg-yellow-50 text-yellow-600 hover:bg-yellow-500 hover:text-white rounded-xl transition-all shadow-sm" title="Edit Data">
                                        ...
                                        <form action="{{ route('doctors.destroy', $doctor->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data dokter {{ $doctor->nama_dokter }}?')">
                                            @csrf @method('DELETE')
                                            <button type="submit" class="p-2 bg-red-50 text-red-600 hover:bg-red-500 hover:text-white rounded-xl transition-all shadow-sm" title="Hapus Data">
                ...
                @if($doctors->isEmpty())
                    <div class="text-center py-20 bg-orange-50/50 rounded-3xl border-2 border-dashed border-orange-200 mt-4">
